...
class MockCalendarServer {
    ...
    createMockApi() {
        return {
            Events: {
                list: jest.fn().mockImplementation((calendarId, options) =>
                    this.listEvents(calendarId, options)
                ),
                get: jest.fn().mockImplementation((calendarId, eventId) =>
                    this.getEvent(calendarId, eventId)
                ),
                insert: jest.fn().mockImplementation((event, calendarId) =>
                    this.addEvent(calendarId, event)
                ),
                update: jest.fn().mockImplementation((event, calendarId, eventId) =>
                    this.updateEvent(calendarId, eventId, event)
                ),
                patch: jest.fn().mockImplementation((event, calendarId, eventId) =>
                    this.updateEvent(calendarId, eventId, event)
                ),
                remove: jest.fn().mockImplementation((calendarId, eventId) =>
