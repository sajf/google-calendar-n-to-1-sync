/**
 * @file Sync state management for preventing infinite loops in bidirectional sync.
...
class SyncStateManager {
  ...
  detectPingPongPattern(sourceCalendarId, targetCalendarId, eventId) {
    ...
    if (recentOperations.length >= 4) {
      ...
      const isAlternating = pattern.every((op, index) => {
        ...
        return op.sourceCalendarId === prevOp.targetCalendarId && op.targetCalendarId === prevOp.sourceCalendarId;
      });

      if (isAlternating) {
        console.log(
          `Ping-pong pattern detected for event ${eventId} between ${sourceCalendarId} and ${targetCalendarId}`
        );
        return true;
      }
    }
  ...
  shouldSkipSync(sourceCalendarId, targetCalendarId, eventId, sourceUpdated, targetUpdated) {
    ...
    if (recentChange.originCalendarId === sourceCalendarId && timeDiff < this.loopDetectionWindow) {
  ...
  getSyncStats() {
    const now = Date.now();
    const recentOperations = this.operationHistory.filter(op => now - op.timestamp < this.loopDetectionWindow);
